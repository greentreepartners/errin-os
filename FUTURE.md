...
**Scope when picked up:** Editable controls (dropdown or cycle pill — decide at design time) for effort, impact, horizon on AdminTaskCard. Add the three fields to AddTaskForm. PATCH handler whitelist needs extension — currently accepts only title, description, status, visibility. Mirror the POST whitelist pattern established in Brief B (c47a6f7): add enum type guards (isHorizon, isEffort, isImpact) and extend ALLOWED_FIELDS.
